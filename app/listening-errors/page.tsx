    ...
    <div className="rounded-2xl border overflow-hidden transition-all duration-200 hover:shadow-md" style={{ borderColor: meta.border, background: 'var(--card)' }}>
      ...
      {expanded && error.transcript && (
        <div className="px-4 pb-4 border-t border-[var(--border)] pt-3">
          ...
          <div className="bg-[var(--bg)] rounded-xl p-3 text-sm leading-relaxed font-mono text-[var(--text)]">
            {error.transcript}
          </div>
        </div>
      )}
    ...
    <div className="min-h-dvh bg-[var(--bg)]">
      ...
      <header className="sticky top-0 z-30 bg-[var(--card)] border-b border-[var(--border)]">
        <div className="max-w-4xl mx-auto px-4 h-14 flex items-center gap-3">
          ...
          <div className="flex-1 min-w-0">
            <h1 className="font-black text-[var(--text)] text-sm truncate">🎧 Nhật Ký Lỗi Listening</h1>
            <p className="text-[10px] text-[var(--text-muted)]">{errors.length} lỗi đã lưu · Luyện VOL → ghi lỗi → khắc phục</p>
          </div>
          <button onClick={() => setShowForm(v => !v)} className="btn-primary px-3 py-1.5 text-xs gap-1">
            ...
            <div className="flex gap-2.5">
              <div className="relative flex-1">
                ...
                <input type="text" value={searchText} onChange={e => setSearchText(e.target.value)}
                  placeholder="Tìm lỗi, đáp án, transcript..." className="q-input pl-9 text-sm"
                />
              </div>
            </div>
            {/* Section filter */}
